...
tables={};
numconf=180;
For[i=0, i<numconf-2, i++,  (*Si poteva anche usare Table*)
    tmp=RandomState[];
...
For[i=1, i<Length[target], i++,
    stacks[[i]]=Take[Reverse[target], i];
    tables[[i]]=Sort[ Drop[Reverse[target],i]]; 

    ttmp=RandomChoice[tables[[i]]];

    stacks[[i+Length[target]-1]]=Append[stacks[[i]], ttmp] ;
    tables[[i+Length[target]-1]]=DeleteCases[tables[[i]], ttmp];
];
...
maxpassi=6*Length[target];
minpassi=Length[target];
...
Nn[]:=Module[{tmp},
     	     If[passi>maxpassi, Return["X"]];    
	     tmp=Tb[];
	     If[Not[MemberQ[target, tmp] || tmp==="NIL"], Return["X"]]; 
	     
	     If[tmp!="NIL", Return[target[[	Flatten[Position[target,tmp]][[1]]   -1	]]	], 
...
Du[exp1_,exp2_]:=Module[{tmp, steps=0},	
	If[passi>maxpassi, Return["X"]]; passi=passi+1;
	If[exp1==="X" || exp2==="X", Return["X"]];
	
	tmp="NIL";
	While[ exp2==="TRUE" && steps<maxdu,
	       If[exp1==="X" || exp2==="X", Return["X"]]; steps++;
	       tmp=exp1]; 
	passi=passi+1;
...
CreateExpression[]:=Module[{i},  (*Forse si puo' mettere Return prima di Which?*)
	i=Random[Integer,{1,11}];                                (*RandomInteger[{1,8}];*)
	Which[
		...
		i<12,    Return[RandomChoice[sensors]]  (*Non il modo piu' raffinato per cambiare le proabilita'*)
...
GenerateRandomProgram[]:=Module[{},
	...
	While[Length[Level[program, -1]]>9,
...
fitnesspars[x_] :=
	Module[{fit, tmp, passitot=0},
	       ...
	       If[passitot<minpassi || passitot>maxpassi,
		  fite=0,
		  fite=passitot*a +c
	       ],
...
fitnesskoza[x_] :=
	Module[{fit, tmp, passitot=0},
	       ...
	       If[passitot<minpassi || passitot>maxpassi || fit<numconf,
		  fite=0,
		  fite=passitot*a +b
	       ];
...
GeneraFigli[ pop_, criterio_, voti_] := Module[ {intervallo,
			      ...
			      figli=Append[figli, Fittest[pop,voti]];
			      figli=Append[figli, Fittest[pop,voti]];
...
(*	       pc = ppcc;
	       ...
	       If[ Max[voti]===150, Return[gencount]];
	       
	       While[ Max[voti] < 100 && gencount<100 ,
		      (*If[Max[voti]>50, fitness[x_]:=fitnesspars[x]];*)
...
(*		      fin=popin[[Position[voti,Max[voti]]]];
		      ...
		      (*If[ Apply[Plus, voti]>60*Nindividui,
			  fitness[x_]:=fitnesspars[x]
		      ];*)

		      If[Max[voti]===100, Print["Soluzione trovata"]];
		      
		      tmpgen=Append[tmpgen,popin];
		      tmpvoti=Append[tmpvoti, voti];
	       ];
